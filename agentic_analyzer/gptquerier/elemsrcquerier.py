...
class ElemSrcQuerier(GPTQuerier):

	def __init__(self, elem_src_formatter, target_config, query_result_template=None):
		self._set_up_query_result = lambda : copy.deepcopy(query_result_template) if query_result_template is not None else {"YES": [], "NO": []}

		super().__init__(elem_src_formatter, target_config)

		print("Your source code analyzer is ready!")
